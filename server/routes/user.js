...
exports.signup = function (req, res, next) {
    ...
    if (!firstname || !lastname || !email || !username || !password) {
        return res.status(422).json({ success: false, message: 'Gönderilen bilgiler kabul edilmez.' });
    }
...
exports.login = function (req, res, next) {
    ...
    User.findOne({ username: req.body.username }, function (err, user) {
        ...
        if (!user) {
            res.status(201).json({ success: false, message: 'Giriş bilgileri hatalı!' });
        } else if (user) {
...
exports.authenticate = function (req, res, next) {
    ...
    if (token) {
        jwt.verify(token, config.secret, function (err, decoded) {
            if (err) {
                return res.status(201).json({ success: false, message: 'Siteme giriş yapılmamış lütfen giriş yapınız.', errcode: 'exp-token' });
            } else {
                req.decoded = decoded;
                next();
            }
            ...
            if (user) {
                user.comparePassword(oldpassword, function (err, isMatch) {
                    if (isMatch && !err) {
                        ...
                        res.status(201).json({ success: false, message: 'Eski parola doğru değil.' });
                    }
                });
            }
        });
